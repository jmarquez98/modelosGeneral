...
def optimizar(techo,piso,step,inv,dataFreq,startDate, serie, isItFred, numResults):
	...
	for i in range(1,len(sp)):
		...
		for bajo in np.arange(piso,techo,step):
			for alto in np.arange(bajo,techo,step):
				if (bajo,alto) not in dic_res:
					dic_res[(bajo,alto)] = [[],[]]
											
				if len(value) > 0:	
					
					dic_res[(bajo,alto)][1].append(infoFechas[i])
					

					if value[-1] > alto:
						if inv:
							dic_res[(bajo,alto)][0].append(0)
						else:	
							dic_res[(bajo,alto)][0].append(1)
						...
						try:
							dic_res[(bajo,alto)][0].append(dic_res[(bajo,alto)][0][-1])
						except:
							dic_res[(bajo,alto)][0].append(0)	
				else:
					try:
